...
export const RecommendationOptionsCard: React.FC<RecommendationOptionsCardProps> =
  memo(({ option, isViewingFull, onViewMore, onSelect, isCreating }) => {
    // Memoize content parsing for performance
    const contentParagraphs = useMemo(() => {
      const contentToDisplay = isViewingFull
        ? option.content
      ...
      <div className='border-2 border-gray-200 rounded-lg p-6 hover:border-blue-300 hover:shadow-md transition-all duration-200'>
        ...
        {option.explanation && (
          <div className='bg-blue-50 p-3 rounded-md mb-4 border border-blue-200'>
            <p className='text-sm text-blue-800 italic'>{option.explanation}</p>
          </div>
        )}
...
RecommendationOptionsCard.displayName = 'RecommendationOptionsCard';

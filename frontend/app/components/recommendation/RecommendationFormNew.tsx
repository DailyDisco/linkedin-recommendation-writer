...
export const RecommendationFormNew: React.FC<RecommendationFormNewProps> = memo(
  ({
    formData,
    errors,
    parsedGitHubInput,
    ...
    const handleWorkingRelationshipChange = useCallback(
      (e: React.ChangeEvent<HTMLTextAreaElement>) =>
        onChange('workingRelationship', e.target.value),
      [onChange]
    );

    const handleSpecificSkillsChange = useCallback(
      (e: React.ChangeEvent<HTMLTextAreaElement>) =>
        onChange('specificSkills', e.target.value),
      [onChange]
    );

    const handleTimeWorkedChange = useCallback(
      (e: React.ChangeEvent<HTMLInputElement>) =>
        onChange('timeWorkedTogether', e.target.value),
      [onChange]
    );

    const handleAchievementsChange = useCallback(
      (e: React.ChangeEvent<HTMLTextAreaElement>) =>
        onChange('notableAchievements', e.target.value),
      [onChange]
    );
    return (
      <form onSubmit={onSubmit} className='space-y-6'>
        ...
        <div>
          ...
          <Textarea
            ...
            onChange={handleWorkingRelationshipChange}
            aria-describedby={
              errors.workingRelationship
          ...
          <p className='mt-1 text-sm text-gray-600'>
            Describe your professional connection, e.g., &ldquo;We collaborated
            on the frontend team for 8 months&rdquo; or &ldquo;I mentored them
            during their internship.&rdquo;
          </p>
        </div>
        ...
        <div>
          ...
          <Textarea
            ...
            onChange={handleSpecificSkillsChange}
            rows={3}
          />
          <p className='mt-1 text-sm text-gray-600'>
        ...
        <div>
          ...
          />
          <p className='mt-1 text-sm text-gray-600'>
            Highlight specific accomplishments or impact, e.g., &ldquo;Reduced
            app load time by 40%&rdquo; or &ldquo;Led the migration to
            microservices architecture.&rdquo;
          </p>
        </div>
...
RecommendationFormNew.displayName = 'RecommendationFormNew';

...
class DatabaseError(BaseApplicationError):
    ...
    def __init__(self, operation: str, message: str, details: Optional[Dict[str, Any]] = None):
        super().__init__(f"Database {operation} failed: {message}", "DATABASE_ERROR", details)
...
class CacheError(BaseApplicationError):
    ...
    def __init__(self, operation: str, key: str, message: str, details: Optional[Dict[str, Any]] = None):
        super().__init__(f"Cache {operation} failed for key '{key}': {message}", "CACHE_ERROR", details)
        self.operation = operation
        self.key = key
...
class RateLimitError(BaseApplicationError):
    ...
    def __init__(self, service: str, limit: str, details: Optional[Dict[str, Any]] = None):
        super().__init__(f"Rate limit exceeded for {service}: {limit}", "RATE_LIMIT_ERROR", details)
        self.service = service
        self.limit = limit
...
class ConfigurationError(BaseApplicationError):
    ...
    def __init__(self, setting: str, message: str, details: Optional[Dict[str, Any]] = None):
        super().__init__(f"Configuration error for '{setting}': {message}", "CONFIGURATION_ERROR", details)

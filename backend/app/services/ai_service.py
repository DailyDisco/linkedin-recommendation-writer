    ...
    def _build_prompt(
        ...
        if commit_analysis and commit_analysis.get("total_commits_analyzed", 0) > 0:
            if github_data.get("repository_info"):
                ...
                tech_contributions = commit_analysis.get("technical_contributions", {})
                if tech_contributions:
                    top_contributions = sorted(tech_contributions.items(), key=lambda x: x[1], reverse=True)[:2]
        ...
        if github_data.get("repository_info"):
            ...
            base_guidelines.insert(3, f"- Focus on skills and technologies they showed in {repo_name}")
            base_guidelines.insert(4, "- Mention the project when it helps explain their contributions")
    ...
    async def regenerate_recommendation(
        ...
        try:
            ...
            result = {
                ...
                "title": self._extract_title(refined_content, github_data["user_data"]["github_username"]),
...
"""
    ...
    def _score_data_completeness(self, github_data: Dict[str, Any]) -> int:
        ...
        if user_data.get("bio"): score += 15
        if user_data.get("public_repos", 0) > 0: score += 20
        if user_data.get("followers", 0) > 0: score += 10
        
        # Repository data
        repositories = github_data.get("repositories", [])
        if repositories: score += 20
        if len(repositories) >= 5: score += 10
        
        # Language diversity
        languages = github_data.get("languages", [])
        if len(languages) >= 3: score += 15
        
        # Skills analysis
        skills = github_data.get("skills", {})
        if skills.get("technical_skills"): score += 10
    ...
    def _score_content_quality(self, content: str) -> int:
        ...
        if 100 <= word_count <= 400: score += 30
        elif 50 <= word_count <= 500: score += 20
        else: score += 10
        
        # Content structure indicators
        sentences = content.split('.')
        if len(sentences) >= 3: score += 20  # Has multiple sentences
        
        # Professional language indicators
        professional_words = ["technical", "skilled", "expertise", "development", "project", "team"]
        if any(word in content.lower() for word in professional_words): score += 25
        
        # Specificity indicators
        specific_indicators = ["github", "repository", "commit", "code", "programming"]
        if any(indicator in content.lower() for indicator in specific_indicators): score += 25

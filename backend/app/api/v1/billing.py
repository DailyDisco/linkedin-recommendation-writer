...
async def get_feature_access(
    ...
    """Get feature access for the authenticated user based on their tier."""
    tier = current_user.effective_tier

    return {
        "tier": tier,
        "features": {
            "multiple_options": current_user.is_pro_or_higher,
            "keyword_refinement": current_user.can_use_keyword_refinement,
            "all_tones": current_user.is_pro_or_higher,
            "version_history": current_user.is_pro_or_higher,
            "api_access": current_user.can_use_api,
            "priority_support": current_user.is_team_or_higher,
        },
        "limits": {
            "daily_generations": current_user.effective_daily_limit,
